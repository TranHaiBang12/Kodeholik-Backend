...
public interface ExamRepository extends JpaRepository<Exam, Long> {
        ...
        @Query("SELECT e FROM Exam e WHERE (cast(:title as text) IS NULL OR (LOWER(e.title) LIKE '%' || LOWER(CAST(:title AS text)) || '%')) AND (COALESCE(:status, e.status) = e.status) AND (e.startTime >= :start AND e.endTime <= :end)")
