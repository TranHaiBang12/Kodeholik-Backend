...
public interface ExamParticipantRepository extends JpaRepository<ExamParticipant, ExamParticipantId> {
        ...
        @Query("SELECT e FROM ExamParticipant e " +
                        "WHERE e.participant = :user " +
                        "AND (CAST(:title AS text) IS NULL OR (LOWER(e.exam.title) LIKE '%' || LOWER(CAST(:title AS text)) || '%')) " +
                        "AND (COALESCE(:status, e.exam.status) = e.exam.status) " +
                        "AND (e.exam.startTime >= :start AND e.exam.endTime <= :end)")
