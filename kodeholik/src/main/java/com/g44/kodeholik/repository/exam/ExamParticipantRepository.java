...
public interface ExamParticipantRepository extends JpaRepository<ExamParticipant, ExamParticipantId> {
        ...
        @Query("SELECT e FROM ExamParticipant e WHERE e.participant = :user AND (cast(:title as text) IS NULL OR (LOWER(e.exam.title) LIKE LOWER(CONCAT('%', :title, '%')) || cast(:title as text) || '%')) AND (COALESCE(:status, e.exam.status) = e.exam.status) AND (e.exam.startTime >= :start AND e.exam.endTime <= :end)")
